...
其中main.go是main包，cmd是下属于cmd包，按照上面文章应该这么传值：
```
go build -ldflags "-s -w -X rtfd/cmd.commitID=xxx -X rtfd/cmd.built=xxx"
```

但是没用，ldflag格式是：`-ldflags="-X 'package_path.variable_name=new_value'"`

可能是新手，没搞明白package_path含义，google也是没找到教新的分享，
捣鼓好久突然灵机一动，改为这么传值：
```
go build -ldflags "-s -w -X tcw.im/rtfd/cmd.commitID=xxx -X tcw.im/rtfd/cmd.built=xxx"
```
